...
public class LatitudeFuserAverage extends AttributeValueFuser<Double, Location, Attribute>{

	public LatitudeFuserAverage() {
		super(new Average<>());
	}

	@Override
	protected Double getValue(Location record, Correspondence<Attribute, Matchable> correspondence) {
		return record.getAddress().getLatitude();
	}
	...
	public void fuse(RecordGroup<Location, Attribute> group, Location fusedRecord,
		...
		fusedRecord.getAddress().setLatitude(fused.getValue());
	}

	@Override
	public boolean hasValue(Location record, Correspondence<Attribute, Matchable> correspondence) {
		return record.hasValue(Location.ADDRESS) && record.getAddress().hasValue(Address.LATITUDE);
	}
