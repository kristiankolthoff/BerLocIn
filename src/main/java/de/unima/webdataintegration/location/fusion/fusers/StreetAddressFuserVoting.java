...
public class StreetAddressFuserVoting extends AttributeValueFuser<String, Location, Attribute>{

	public StreetAddressFuserVoting() {
		super(new Voting<>());
	}

	@Override
	protected String getValue(Location record, Correspondence<Attribute, Matchable> correspondence) {
		return record.getAddress().getStreetAddress();
	}
	...
	public void fuse(RecordGroup<Location, Attribute> group, Location fusedRecord,
		...
		fusedRecord.getAddress().setStreetAddress(fused.getValue());
	}

	@Override
	public boolean hasValue(Location record, Correspondence<Attribute, Matchable> correspondence) {
		return record.hasValue(Location.ADDRESS) && record.getAddress().hasValue(Address.STREET_ADDRESS);
	}
